...
* **Native macOS SwiftUI Configurator (`Antiknob.app`)**:
  ...
  * **Bottom Status Bar**: Daemon status (square, green/orange), hardware connection (circle, transport-colored), transport, and power source read as glyphs in the lower-right corner (words in the tooltip), next to a manual refresh and the transient autosave badge.
